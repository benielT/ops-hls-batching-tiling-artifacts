...
PROFILE_DIR=./tapa/profile_data/${TARGET_MODE}/
...
for params in "${parameter_sets[@]}"; do
    ...
    if [[ "${CXXFLAGS}" == *"-DPOWER_PROFILE"* ]]; then
        ...
        if [[ $TARGET_MODE == sw_emu || $TARGET_MODE == hw_emu ]]; then
            ...
            XCL_EMULATION_MODE=${TARGET_MODE} ${SCRIPT_DIR}/tapa/build/${TARGET_MODE}/${APP_NAME}_host ${SCRIPT_DIR}/tapa/build/${TARGET_MODE}/${APP_NAME}.xclbin -sizex="${sizex}" -sizey="${sizey}" -iters="${iters}" -batch="${batch}" -bsize="${bsize}"
        elif [[ $TARGET_MODE == tapa_sw_emu ]]; then
            echo "Running in emulation mode with ${TARGET_MODE}"
            emconfigutil --platform xilinx_u280_gen3x16_xdma_1_202211_1 --nd 1
            GLOG_logtostderr=1 GLOG_v=3 XCL_EMULATION_MODE=sw_emu ${SCRIPT_DIR}/tapa/build/${TARGET_MODE}/${APP_NAME}_host -sizex="${sizex}" -sizey="${sizey}" -iters="${iters}" -batch="${batch}" -bsize="${bsize}" --trace
        elif [[ $TARGET_MODE == tapa_hw_emu ]]; then
            echo "Running in emulation mode with ${TARGET_MODE}"
            emconfigutil --platform xilinx_u280_gen3x16_xdma_1_202211_1 --nd 1
            GLOG_logtostderr=1 GLOG_v=3 XCL_EMULATION_MODE=hw_emu ${SCRIPT_DIR}/tapa/build/${TARGET_MODE}/${APP_NAME}_host ${SCRIPT_DIR}/tapa/build/${TARGET_MODE}/${APP_NAME}.xclbin -sizex="${sizex}" -sizey="${sizey}" -iters="${iters}" -batch="${batch}" -bsize="${bsize}" --trace
        else
            echo "Running HW mode"
            ${SCRIPT_DIR}/tapa/build/${TARGET_MODE}/${APP_NAME}_host ${SCRIPT_DIR}/tapa/build/${TARGET_MODE}/${APP_NAME}.xclbin -sizex="${sizex}" -sizey="${sizey}" -iters="${iters}" -batch="${batch}" -bsize="${bsize}"
        fi
    fi
