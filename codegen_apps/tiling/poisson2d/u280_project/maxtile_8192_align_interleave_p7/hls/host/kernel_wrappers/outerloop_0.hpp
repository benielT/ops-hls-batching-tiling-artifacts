// Auto-generated at 2026-08-18 17:01:50.521508 by ops-translator
...
public:
    KernelWrapper_outerloop_0():
            ...
            m_kernelName2("kernel_outerloop_0_2"),
            m_datamoverName("datamover_outerloop_0")
    {
        ...
        OCL_CHECK(err, m_kernel_2 = cl::Kernel(m_fpga->getProgram(), m_kernelName2.c_str(), &err));
    ...
    )
    {
        ...
        ops::hls::SizeType2d tile_size = {get_tile_size_x(), 1};
        ops::hls::SizeType2d overlap_size = {get_overlap_size_x<7, 1, mem_vector_factor>(), 
0};
        ...
        {
            const unsigned short banks = OPS_HLS_TILE_BANKS;
            printf("[FIX2][WRAP] tile=%u eff=%u ovl=%u last=%u count=%u | eff%%banks=%u %s\n",
                   (unsigned)tile_size[0], (unsigned)effective_tile_size[0],
                   (unsigned)overlap_size[0], (unsigned)last_tile_size[0],
                   (unsigned)tile_count[0],
                   (unsigned)(effective_tile_size[0] % banks),
                   (effective_tile_size[0] % banks) ? "*** MISALIGNED ***" : "aligned");

            for (unsigned short t = 0; t < tile_count[0]; t++) {
                const unsigned short off = t * effective_tile_size[0];
                const unsigned short sz  = (t == tile_count[0] - 1)
                                         ? last_tile_size[0] : tile_size[0];
                printf("[FIX2][WRAP]  tile %2u: offset=%5u start_bank=%u size=%u rem=%u end=%u\n",
                       t, (unsigned)off, (unsigned)(off % banks), (unsigned)sz,
                       (unsigned)(sz % banks), (unsigned)(off + sz));
            }
            const unsigned int end   = (tile_count[0] - 1) * effective_tile_size[0]
                                     + last_tile_size[0];
            const unsigned int tiled = (unsigned int)(original_gridSize_copy[0]
                                                    - read_range_copy.start[0]);
            const unsigned int data  = (unsigned int)(read_range_copy.end[0]
                                                    - read_range_copy.start[0]);
            printf("[FIX2][WRAP] last tile ends at %u, tiled_xblocks=%u %s"
                   " | data_xblocks=%u grid_pad=%u | last_tile_upper_limit_x=%u\n",
                   end, tiled, (end == tiled) ? "" : "*** MISMATCH ***",
                   data, tiled - data, (unsigned)last_tile_upper_limit_x);
        }
    #endif
#endif
#ifdef DEBUG_LOG
		...
		unsigned int total_iter_par_factor = 7;
